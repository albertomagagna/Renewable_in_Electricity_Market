...
upward_regulation_service = value(dual(power_balance_equation)).+ 0.1*[generator_data["p_G$g"].cost for g in 1:G] # 10% additional to day-ahead price for overproducing
downward_regulation_service =value(dual(power_balance_equation)).- 0.15*[generator_data["p_G$g"].cost for g in 1:G]# -15%  to day-ahead price for underproducing
load_curtailment_cost = 20 # cost for load shedding


##____________________Find generators participating in the balancing market____________________
...
gen_Participating_positions = findall(x -> x == 0, gen_NON_partecipating) # Find the positions of PARTICIPATING generators
...
demand_NON_partecipating_position = findall(x -> x == 1, demand_NON_partecipating) # Find the positions of non-participating demands
...
@constraint(Step5_balancing, [g in gen_NON_partecipating_position], generator_upward_reg[g] == 0)
...
power_balance_equation_balancing= @constraint(Step5_balancing, sum(generator_upward_reg[g] for g in 1:G) 
                                                            ...
                                                            + sum(demand_curtailment[l] for l in 1:L) == -301)
...
power_UP_g =[generator_data["p_G$g"].max_power - value(generator_var[g]) for g in gen_Participating_positions]
cost_UP_g = [upward_regulation_service[g] for g in gen_Participating_positions]
power_DOWN_g =[-value(generator_var[g]) for g in gen_Participating_positions]
cost_DOWN_g = [-downward_regulation_service[g] for g in gen_Participating_positions]
...
Market_clearing_quantity_balancing =  sum(value(generator_upward_reg[g]) for g in 1:G) - sum(value(generator_downward_reg[g]) for g in 1:G) + sum(value(demand_curtailment[l]) for l in 1:L)
# Plot supply and demand curves
...
# Add text annotations
annotate!([(Market_clearing_quantity_balancing -100, 2.5, text("$(round(Market_clearing_quantity_balancing)) MW", :grey, 8, rotation=90))])
annotate!([(-400, Market_clearing_price_balancing +0.5 , text("$(round(Market_clearing_price_balancing, digits=2)) \$/MWh", :grey, 8))])
